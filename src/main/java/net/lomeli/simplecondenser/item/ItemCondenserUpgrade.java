...
public class ItemCondenserUpgrade extends ItemSC {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        AlchemicalType type = AlchemicalType.getType(stack.getItemDamage() + 1);
        ...
        if (tile != null && tile instanceof TileCondenserBase) {
            if (!world.isRemote) {
                ...
                if (type.getIndex() == condenserType.getIndex() + 1) {
                    NBTTagCompound tag = new NBTTagCompound();
                    condenser.writeToNBT(tag);
                    condenser.readFromNBT(new NBTTagCompound());
                    tag.setInteger("CondenserType", type.getIndex());
                    world.setBlockToAir(x, y, z);
                    world.setBlock(x, y, z, type.getIndex() == 1 ? ModBlocks.azureCondenser : ModBlocks.miniumCondenser, 0, 2);
    ...
    public void registerIcons(IIconRegister register) {
        ...
        for (int i = 0; i < icons.length; i++)
            icons[i] = register.registerIcon(SimpleCondenser.MOD_ID + ":condenserUpgrade" + AlchemicalType.getType(i + 1).getName());
    }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean something) {
        list.add(StatCollector.translateToLocal("item.simplecondenser.condenserUpgrade." + AlchemicalType.getType(stack.getItemDamage() + 1).getName() + ".desc"));
    }
    ...
    public String getUnlocalizedName(ItemStack stack) {
        return this.getUnlocalizedName() + "." + AlchemicalType.getType(stack.getItemDamage() + 1).getName();
    }
}
